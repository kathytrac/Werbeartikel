    ...
    return Class.extend({
        ...
        initNextPageClickAction: function() {
            ...
            this.em.find('button.next').click(function(){
                ...
                var validated = true;
                $(element).parents('.formpage').find('input,select,textarea').each(function () {
                    if(!$(this).valid()){
                        validated = false;
                    }
                });
        ...
        renderDateElements: function() {
            ...
            //date & time
            form.find('.date.dateandtime').datetimepicker({
            ...
            //only date
            form.find('.date.dateonly').datetimepicker({
                allowInputToggle: true,
                format:'L',
                locale: locale
            });
        ...
        getDropzoneConfig: function(em) {
            ...
            return {
                url: this.config.uploadUrl + 'upload_element_index/' + uploadElementNumber,
                addRemoveLinks: true,
                maxFiles: maxFiles,
        ...
        initReCaptcha: function() {
            ...
            window.reCaptchaLoaded = _.bind(function() {
        ...
        renderReCaptchaElements: function() {
            this.form.find('.g-recaptcha').each(_.bind(function (k, em) {
                try {
                    ...
                    $(em).remove();
                    grecaptcha.render(newCaptcha[0], {sitekey: this.config.recaptchaKey})
                } catch (err) {
                    console.log(err);
                }
            }, this));
