            ...
            try {
                ...
                foreach ($storeIds as $storeId) {
                    ...
                    if (!$this->validateEntitiesByStore($data, $storeId)) {
                        $message = __('It is impossible to save redirect for %storeName Store', ['storeName' => $storeName]);
...
//                        $this->urlFinder->findAllByData()
                    ...
                    if ($successFlag) {
                        $successMessage = __('The redirect for %storeName Store has been saved.', ['storeName' => $storeName]);
    ...
    protected function prepareData($data)
    {
        ...
        if (!empty($data[$requestKey])) {
            ...
            InputException::requiredField($redirectTypeEntityOptions[$data[CustomRedirectInterface::REQUEST_ENTITY_TYPE]]);
        }
        ...
        if (!empty($data[$targetKey])) {
            ...
            InputException::requiredField($redirectTypeEntityOptions[$data[CustomRedirectInterface::TARGET_ENTITY_TYPE]]);
        }
